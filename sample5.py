...
# PO1 element descriptions for better user interaction
PO1_ELEMENT_DESCRIPTIONS = {
    1: "Assigned Identification",
    2: "Quantity",
    3: "Unit of Measure Code",
    4: "Unit Price",
    5: "Basis of Unit Price Code",
    6: "Product ID Qualifier 1",
    7: "Product ID 1",
    8: "Product ID Qualifier 2",
    9: "Product ID 2",
    10: "Product ID Qualifier 3",
    11: "Product ID 3",
    12: "Product ID Qualifier 4",
    13: "Product ID 4",
    14: "Product ID Qualifier 5",
    15: "Product ID 5",
    16: "Product ID Qualifier 6",
    17: "Product ID 6",
    18: "Product ID Qualifier 7",
    19: "Product ID 7",
    20: "Product ID Qualifier 8",
    21: "Product ID 8"
}

def find_config_file(filename="conf.json"):
    """Find the configuration file in the script directory."""
...
def adjust_date(date_str, config, segment_type):
    """Adjust date based on configuration."""
...
def get_element_description(index):
    """Get a user-friendly description for a PO1 element."""
    return PO1_ELEMENT_DESCRIPTIONS.get(index, f"Element {index}")
...
def select_po1_segments(po1_segments):
    ...
    print("\n" + "="*60)
    print("PO1 SEGMENT SELECTION")
    print("="*60)
    print("\nListing PO1 segments with checkboxes (select by sequence numbers):")
    for i, segment in enumerate(po1_segments, 1):
        print(f"[ ] PO_sequence {i}: {segment[:80]}..." if len(segment) > 80 else f"[ ] PO_sequence {i}: {segment}")
    ...
    while True:
        ...
        try:
            ...
            for seq_num, segment in selected_segments:
                print(f"[✓] PO_sequence {seq_num}: {segment[:80]}..." if len(segment) > 80 else f"[✓] PO_sequence {seq_num}: {segment}")
...
def get_user_input_for_po1_elements(selected_po1_segments):
    ...
    for seq_num, po1_line in selected_po1_segments:
        ...
        print("\n" + "="*60)
        print(f"EDITING PO1 SEGMENT (SEQUENCE {seq_num})")
        print("="*60)
        print(f"Segment has {num_elements} elements")
        print(f"Original: {po1_line}")
        new_elements = {}

        # Start from element 2 (index 2) since element 1 is the segment identifier "PO1"
        for idx in range(2, num_elements):
            current_value = parts[idx]
            element_name = get_element_description(idx)
            
            print(f"\n[ ] Element {idx} - {element_name}: {current_value}")
            user_input = input(f"Enter new value for Element {idx} - {element_name} (press Enter to keep '{current_value}'): ").strip()

            if user_input:
                # Special validation for Quantity (element 2)
                if idx == 2:
                    try:
                        qty = int(user_input)
                        if not (0 <= qty <= 10):
                            ...
                            print(f"[✓] Element {idx} - {element_name}: {current_value} (unchanged)")
                        else:
                            print(f"New quantity set: {qty}")
                            new_elements[idx] = str(qty)
                            print(f"[✓] Element {idx} - {element_name}: {qty} (updated)")
                    except ValueError:
                    ...
                    print(f"[✓] Element {idx} - {element_name}: {user_input} (updated)")
            else:
                print(f"Keeping original value: {current_value}")
                new_elements[idx] = None
                print(f"[✓] Element {idx} - {element_name}: {current_value} (unchanged)")

        print(f"\n[✓] PO1 Segment (Sequence {seq_num}) editing completed.")
...
def modify_edi_file(content, config, selected_segments=None, new_elements_list=None, is_bulk_processing=False, file_counter=None):
    """Modify EDI file content based on configuration and user inputs."""
    selected_segments = selected_segments or []
    new_elements_list = new_elements_list or []
    
    # Detect file format (single-line or multi-line)
    ...
    # Group PO1 segments with their related segments
    po1_groups = []
    current_group = []
    po1_index = 0
    ...
    # Process selected PO1 segments and other segments
    ...
    for index, group in po1_groups:
        if group[0].startswith('PO1*'):
            # Only include PO1 groups that are selected for editing
            if selected_segments and index in selected_seq_nums:
                po1_counter += 1
                po1_line = group[0]
                parts = po1_line.split('*')
                # Keep the original sequence number (parts[1]) unchanged
                ...
                if index in new_elements_dict:
                    for idx, value in new_elements_dict[index].items():
                        if value is not None and idx < len(parts):
                            parts[idx] = value
            ...
            # Skip unselected PO1 groups when there are selected segments
            elif selected_segments:
                print(f"Skipping unselected PO1 segment (sequence {index})")
                continue
            # Include all PO1 groups when no segments are selected (single file case)
            else:
                filtered_lines.extend(group)
        else:
    ...
    # Add missing CTT and SE segments if needed
    ...
    # Return the modified content in the same format as the input
    if is_single_line:
...
def process_files_and_save(config):
    """Process all EDI files in the input folder and save them to the output folder."""
    ...
    print("="*80)
    print(f"PROCESSING {len(input_files)} EDI FILES")
    print("="*80)
    ...
    for file_path in input_files:
        if os.path.isfile(file_path):
            print(f"\n{'-'*60}")
            print(f"PROCESSING FILE {file_counter}/{len(input_files)}: {os.path.basename(file_path)}")
            print(f"{'-'*60}")
            try:
                with open(file_path, 'r', encoding='utf-8') as file:
                    content = file.read()

                # Detect file format and identify transaction type
                is_single_line = '\n' not in content and '*' in content
                if is_single_line:
                    lines = [line.strip().rstrip('~') for line in content.split('~') if line.strip()]
                else:
                ...
                # For bulk processing, require PO1 updates
                if is_bulk_processing:
                ...
                # For single file, skip PO1 processing entirely
                else:
...
if __name__ == '__main__':
    try:
        print("\n" + "="*60)
        print("EDI FILE PROCESSOR".center(60))
        print("="*60)
        print("This utility processes EDI files with dynamic PO1 segment handling")
        print("="*60 + "\n")
