...
def get_user_input_for_po1_elements(selected_po1_segments):
    """Prompt user for specific elements in selected PO1 segments, with checkboxes."""
    element_indices = [2, 6, 7, 8, 9, 10, 11, 12, 13]  # Indices for 48, UP, 070501064863, VA, 64863, CB, 0862021, BO, 000
    new_elements_list = []

    for i, (seq_num, po1_line) in enumerate(selected_po1_segments, 1):
        parts = po1_line.split('*')
        print(f"\n[ ] PO1 Segment (Sequence {seq_num}): {po1_line}")
        new_elements = {}
        ...
        print(f"[✓] PO1 Segment (Sequence {seq_num}) completed.")
        new_elements_list.append(new_elements)

    return new_elements_list

def switch_po1_segments(selected_po1_segments, new_elements_list):
    """Prompt user to switch the order of selected PO1 segments."""
    if len(selected_po1_segments) <= 1:
        print("Only one or no PO1 segments selected. No switching needed.")
        return selected_po1_segments, new_elements_list

    print("\nCurrent selected PO1 segment order:")
    for i, (seq_num, segment) in enumerate(selected_po1_segments, 1):
        print(f"Position {i}: Sequence {seq_num} - {segment}")

    print(f"\nEnter new positions for each selected PO1 segment (1 to {len(selected_po1_segments)}). Press Enter to keep current order.")
    new_order = []
    for i, (seq_num, _) in enumerate(selected_po1_segments, 1):
        while True:
            user_input = input(f"New position for PO1 Segment (Sequence {seq_num}) (current position: {i}): ").strip()
            if user_input == "":
                new_order = list(range(1, len(selected_po1_segments) + 1))  # Keep original order
                print("Keeping original selected PO1 segment order.")
                return selected_po1_segments, new_elements_list
            try:
                pos = int(user_input)
                if 1 <= pos <= len(selected_po1_segments) and pos not in new_order:
                    new_order.append(pos)
                    break
                else:
                    print(f"Invalid position. Enter a unique number between 1 and {len(selected_po1_segments)}.")
            except ValueError:
                print("Invalid input. Enter a number or press Enter to keep order.")

    # Validate that all positions are covered
    if sorted(new_order) != list(range(1, len(selected_po1_segments) + 1)):
        print("Error: Not all positions were specified correctly. Keeping original order.")
        return selected_po1_segments, new_elements_list

    # Reorder segments and element updates
    reordered_segments = [None] * len(selected_po1_segments)
    reordered_elements = [None] * len(new_elements_list)
    for old_pos, new_pos in enumerate(new_order, 1):
        reordered_segments[new_pos - 1] = selected_po1_segments[old_pos - 1]
        reordered_elements[new_pos - 1] = new_elements_list[old_pos - 1]

    print("\nNew selected PO1 segment order:")
    for i, (seq_num, segment) in enumerate(reordered_segments, 1):
        print(f"Position {i}: Sequence {seq_num} - {segment}")

    return reordered_segments, reordered_elements

def select_po1_segments(po1_segments):
    """Prompt user to select PO1 segments by sequence number."""
    if not po1_segments:
        return []

    print("\nSelect PO1 segments to update (enter sequence numbers, e.g., '1,3,5'). Press Enter to select none:")
    while True:
        ...
        try:
            ...
            print("Selected PO1 segments:")
            for seq_num, segment in selected_segments:
                print(f"Sequence {seq_num}: {segment}")
...
def modify_edi_file(content, config, is_bulk_processing=False, file_counter=None):
    ...
    for line in lines:
        ...
        if list_of_strings[0] == 'PO1':
            ...
            po1_segments.append(line)
            po1_sequence += 1

    po1_count = len(po1_segments)
    print(f"\nFound {po1_count} PO1 segments in the file.")

    # Prompt user to select PO1 segments
    selected_po1_segments = select_po1_segments(po1_segments)
    new_elements_list = []

    if selected_po1_segments:
        print("\nPrompting for PO1 element updates...")
        new_elements_list = get_user_input_for_po1_elements(selected_po1_segments)
        # Prompt for switching selected PO1 segments
        selected_po1_segments, new_elements_list = switch_po1_segments(selected_po1_segments, new_elements_list)
    ...
    selected_seq_nums = [seq_num for seq_num, _ in selected_po1_segments]
    selected_po1_lines = [line for _, line in selected_po1_segments]
    ...
    for line in body_lines:
        if line.startswith('PO1*'):
            po1_index += 1
            # Check if this PO1 segment is selected
            if po1_index in selected_seq_nums:
                # Use the reordered selected PO1 segment
            ...
            elif po1_index in selected_seq_nums and new_elements_list:
                # Apply user-provided elements
                user_elements = new_elements_list[selected_seq_nums.index(po1_index)]
